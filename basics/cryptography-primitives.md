...
### ElGamal encryption[​](https://wiki.findora.org/docs/modules/cryptography/primitives#elgamal-encryption) <a href="#elgamal-encryption" id="elgamal-encryption"></a>

The ElGamal encryption in the Zei library is defined over the Ristretto curve, where the base $$\mathbf{G}G$$ is the base point of the Ristretto group. Note that the message $$\mathbf{m}m$$ is encoded into a group element as $$\mathbf{m}\cdot \mathbf{G}m⋅G$$, which means that it can only be decrypted through brute-force. One who wants to remove this restriction can use reversible encoding, but it is not implemented in the Zei library.
...
### Hybrid encryption[​](https://wiki.findora.org/docs/modules/cryptography/primitives#hybrid-encryption) <a href="#hybrid-encryption" id="hybrid-encryption"></a>
...
### Matrix Sigma protocol[​](https://wiki.findora.org/docs/modules/cryptography/primitives#matrix-sigma-protocol) <a href="#matrix-sigma-protocol" id="matrix-sigma-protocol"></a>

The matrix Sigma protocol in the Zei library is a proof of knowledge for the following statement: the prover $$\mathbb{P}P$$ knows a scalar vector $$\vec{x}\in\{0,1\}^n$$ such that:
...
### Schnorr signature[​](https://wiki.findora.org/docs/modules/cryptography/primitives#schnorr-signature) <a href="#schnorr-signature" id="schnorr-signature"></a>
...
* $$\mathsf{KeyGen}(1^\lambda)\rightarrow(\mathsf{sk},\mathsf{pk}):$$
  ...
  * output $$(\mathsf{sk},\mathsf{pk})$$
* $$Sign(sk,m)→σ:$$​
  * $$\mathsf{r}\leftarrow\!{\footnotesize \$} \hspace{3pt}\mathbb{F}$$​
...
* $$\mathsf{Verify}(\mathsf{pk},\mathbf{m},\sigma) := \mathbf{b}\in\{0,1\}$$:
  ...
  * append the message $$\mathbf{m}m$$, the public key $$\mathsf{pk} := \mathsf{sk}\cdot \mathbf{G}$$, and $$\mathbf{R}$$ to the transcript
  * squeeze a challenge $$\mathbf{c}$$ from the Fiat-Shamir transform
  * check $$\mathbf{R} + \mathbf{c}\cdot \mathsf{pk} \stackrel{?}{=} \mathbf{s}\cdot \mathbf{G}$$

### Pedersen commitment over Ristretto[​](https://wiki.findora.org/docs/modules/cryptography/primitives#pedersen-commitment-over-ristretto) <a href="#pedersen-commitment-over-ristretto" id="pedersen-commitment-over-ristretto"></a>
...
### Chaum-Pedersen proof of commitment equality[​](https://wiki.findora.org/docs/modules/cryptography/primitives#chaum-pedersen-proof-of-commitment-equality) <a href="#chaum-pedersen-proof-of-commitment-equality" id="chaum-pedersen-proof-of-commitment-equality"></a>
...
* $$\mathsf{Prove}(\mathbf{comm}_1,\mathbf{comm}_2,\mathbf{m},\mathbf{r}_1,\mathbf{r}_2)\rightarrow \pi$$:
  *   let matrix $$\mathbf{M}$$ be:
      ...
      $$\mathbf{M} := \left( \begin{array}{ccc} \mathbf{G} & \mathbf{H} & 1_\mathbb{G}\\ \mathbf{G} & 1_\mathbb{G} & \mathbf{H} \end{array}\right)$$
  *   let vector $$\vec{\mathbf{x}}$$ be:

      $$\vec{\mathbf{x}} := \left(\begin{array}{lll} \mathbf{m} & \mathbf{r}_1 & \mathbf{r}_2 \end{array}\right)$$
